...
function normalizeFundNameForLookup(name) {
  return String(name || '')
    .replace(/\s+/g, '')
    .replace(/[（(].*?[）)]/g, '')
    .replace(/已?清仓/g, '')
...
function parseAlipayFundTable(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
      ...
      if (i + 1 < lines.length) {
        ...
        if (decimals.length >= 3) {
          ...
          if (decimals.length >= 4) {
            ...
            // OCR 漏掉一列时，优先保住持仓金额/持有收益/累计收益，日收益后续再结合百分比估算。
    ...
    // 检查百分比行（如"占 比 4.83% +0.349%"）
    if (line.includes('占比') && currentFund) {
      const percentages = line.match(/[+-]?\d+\.?\d*%/g);
      if (percentages && percentages.length >= 2) {
        currentFund.positionPercent = percentages[0];
        currentFund.changePercent = percentages[1];

        const numericChangePercent = toNumber(percentages[1].replace('%', ''));
...
async function enrichParsedFunds(funds) {
...
// 加载基金名称到代码的映射（从positions表获取）
async function loadCodeFixMap() {
  try {
    // 查询所有基金记录（is_fund = true）
  ...
  if (req.method === 'POST' && req.url === '/api/upload-fund-screenshot') {
    ...
    req.on('end', async () => {
      try {
        ...
        // 查找模式：20,xxx.xx 并校正为 26,xxx.xx（当上下文是基金金额时）
        ...
        if (fundInfo?.funds?.length) {
          ...
          await enrichParsedFunds(fundInfo.funds);
        } else if (fundInfo?.code && fundInfo.positionValue > 0) {
          await enrichParsedFunds([fundInfo]);
